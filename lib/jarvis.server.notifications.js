...
module.exports = {
	...
	handleNotification(stateId, state) {
		if (stateId && stateId.indexOf('.addNotification') !== -1 && state && state.ack !== true && state.val) {
			...
			clients.forEach(clientId => {
				...
				this.adapter.getState('clients.' + client.ns + '.connected', (err, state) => {
					...
					if (!err && state && state.val === true) {
						...
						this.send(clientId, 'notification', notification); // credentials / clientId, action, data = {}, messageId = _uuid(), key = 'message'
					}
