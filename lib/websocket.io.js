...
class ioWebSocketClient {
	 ...
	 *
	 */
	unsubscribe(messageId, subscriptionKey, stateId) {
		
		if (subscriptionKey && this._subscribeHistory[subscriptionKey]) {
			delete this._subscribeHistory[subscriptionKey];
		}
		else if (stateId && this.subscribeState[stateId]) {
			delete this.subscribeState[stateId];
		}
		
		this.emit(messageId, { subscriptionKey, stateId, 'err': null });
	}
	...
	getHistory(messageId, subscriptionKey, stateId, options = {}) {
		this._adapter.getHistory(stateId, options, (err, history) => {
			if (!err && history) {
				...
				if (this._subscribeHistory[subscriptionKey] && hash !== this._subscribeHistory[subscriptionKey].hash) {
