...
class ioWebSocket extends EventEmitter {
	...
	connect() {
		...
		this.server.listen(this._options.port, () => {
			this._adapter.log.info('WebSocket opening on port ' + this._options.port + (this._options.certificates !== null ? ' using https' : ' using http') + '...');
		});
	}
}
...
class ioWebSocketClient {
	...
	constructor(adapter, client, parentMessage) {
		...
		setInterval(() => {
			if (this._isAlive < (Date.now() - (30 * 1000))) {
		...
		setInterval(() => {
			for (const messageId in this._subscribeHistory) {
				const { stateId, options } = this._subscribeHistory[messageId];
	 ...
	 *
	 */
	setState(messageId, stateId, value, ack = false) {
		this._adapter.setForeignState(stateId, value, ack, err => {
			this.emit(messageId, { err });
		});
	}
	...
	subscribeHistory(messageId, stateId, options) {
		...
		// first subscription
		if (!this._subscribeHistory[messageId]) {
			this._subscribeHistory[messageId] = { messageId, stateId, options };
			this.getHistory(messageId, stateId, options);
		}
		...
		// already subscribed, send cache
			...
			this.emit(messageId, {
				'err': null,
				'history': this._subscribeHistory[messageId].history
			});
		}
	}
	...
	getHistory(messageId, stateId, options = {}) {
		this._adapter.getHistory(stateId, options, (err, history) => {
			if (!err && history) {
				const hash = _hash(history);
				
				if (hash !== this._subscribeHistory[messageId].hash) {
					this._subscribeHistory[messageId].history = history;
					this._subscribeHistory[messageId].hash = hash;
