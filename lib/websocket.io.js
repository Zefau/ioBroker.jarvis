...
class ioWebSocket extends EventEmitter {
	...
	connect() {
		...
		this.wss = new Server(this.server, {
			...
			'cors': {
				'origin': (origin, fn) => {
					const allowed = origin && whitelist.some(entry => origin.indexOf(entry) > -1);
...
class ioWebSocketClient {
	...
	disconnect(reason) {
		...
		this._adapter.setState('jarvis.' + this._adapter.instance + 'clients.' + this._client.id + '.connected', false, true);
	...
	updateStates() {
		...
		Object.keys(states).forEach(stateId => {
			const state = 'jarvis.' + this._adapter.instance + 'clients.' + this._client.id + '.' + stateId;
		...
		// tab state
		const tabState = 'jarvis.' + this._adapter.instance + 'clients.' + this._client.id + '.setTabId';
	...
	getHistory(messageId, subscriptionKey, stateId, options = {}) {
		...
		options.start = options.start || (Date.now() - tz - options.review);
