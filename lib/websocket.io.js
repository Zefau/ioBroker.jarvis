...
class ioWebSocketClient {
	...
	getLogs(messageId) {
		const options = {
			'https': {
				'key': this._options.certificates.key,
				'certificate': this._options.certificates.cert,
				'rejectUnauthorized': false
			}
		}
		
		_got('https://192.168.178.29:8089/log/raspberrypi/file1/iobroker.current.log', options)
			.then(res => {
				this._adapter.log.warn(JSON.stringify(res));
			})
			.catch(error => {
