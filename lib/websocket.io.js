...
class ioWebSocket extends EventEmitter {
	...
	constructor(adapter = null, options = {}) {
		...
		*/
		...
		this._adapter.on('stateChange', (stateId, state) => {
			...
			// scripts
			if (stateId.startsWith('javascript.0.scriptEnabled.') && state && state.ack === true) {
	...
	eventFromClient(msg) {
		...
		if (msg.event === 'getAdapterInstances' || msg.event === 'getScriptStatuses') {
			...
			if (this.sockets[clientId] && this.sockets[clientId]._subscribeOthers && this.sockets[clientId]._subscribeOthers[key] && this.cache[key]) {
...
class ioWebSocketClient {
	...
	readFile(messageId, file) {
		file = _path.resolve(file.replace('{dirname}', __dirname));
		
		_fs.readFile(file, 'utf-8', (err, data) => {
